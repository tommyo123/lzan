; ===========================================================================
; Legal-only variant of upkr-pfusik.s (no undocumented opcodes): the one
; `LAX (u_probs),Y` illegal load (in the hot u_getBit prob decoder) is expanded
; to `LDA (u_probs),Y` / `TAX` (identical A, X, Z and N), costing 1 byte over
; the standard body. Decodes the same stream (lzan::upkr::compress_upkr_6502).
; Use when the decruncher must avoid NMOS illegal opcodes.
...
;
; The 319-byte probs buffer is page-aligned scratch RAM (probs_ram).
; full_decomp seeds bitBuf/src/dest/probs/state from u_inittab, then falls into
; the probs fill.
; ===========================================================================
