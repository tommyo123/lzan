; ===========================================================================
; Subsizer 0.6 standalone (memory-mode) 6502 decruncher, in asm6502 syntax.
...
;
; The tables live in zeroed scratch RAM at $0334; the decoder fills them at
; runtime, so no bytes need emitting (N_PARTS=16, HAVE_LONG_PARTS=1):
...
;
; Structure notes:
;   * dc_ptr SMC operand is assembled to comp_data+comp_data_len (one past the
;     last stream byte; dc_get_byte pre-decrements), so the routine is one-shot
;     (@smc: yes) with no runtime entry seed.
;   * the get_bit refills are factored into one `get_bit` subroutine (bit in C;
;     A/X/Y preserved, A via PHA/PLA around the refill).
;   * get_bits_max8 and get_bits_max16 are merged into one shared `gbs` loop;
;     the masked variant (gb4, the offset-class selector) stays inline. The
;     4-bit table-header reads also route through gbs.
;   * Entry `subsizer_decrunch` == `decrunch`. `comp_data_len` is supplied by
;     the emitter/test.
; ===========================================================================
;@format: subsizer
;@direction: backward
...
; Pre-decrements a 16-bit pointer then loads. The compressed payload is at
; comp_data (length comp_data_len, supplied by the harness); the SMC operand
; is ASSEMBLED to comp_data+comp_data_len (one past the last stream byte), so
; no runtime seeding is needed (the routine is one-shot).
; ---------------------------------------------------------------------------
dc_get_byte:
        LDA dc_ptr
        BNE dcgb_skp1
...
dc_ptr:
        .word comp_data + comp_data_len
        RTS

; ---------------------------------------------------------------------------
; get_bit: shift the next stream bit into C, refilling the marker-bit
; reservoir (buf_zp) from dc_get_byte when it runs empty. A, X, Y preserved.
; ---------------------------------------------------------------------------
get_bit:
        ASL buf_zp
        BNE gb_rts
; C=1 (marker bit shifted out)
        PHA
        JSR dc_get_byte
        ROL
        STA buf_zp
...
; ---------------------------------------------------------------------------
; gbs: read Y bits MSB-first into hibits_zp:A (caller seeds A, and hibits_zp
; when it matters). Enter at gbs_entry with the Z flag reflecting Y (a caller
; LDY survives the JSR), or at gbs_lp when Y != 0. Exits with Y=0 and C=0
; (only 0-bits ever shift out of hibits_zp for the <=16-bit fields of this
; format); Y=0 entry preserves C.
; ---------------------------------------------------------------------------
gbs_lp:
        JSR get_bit
        ROL
...
gbs_entry:
        ...
        RTS                     ; shared: gbs exit AND the decrunch exit
...
dc_lp01:
; get 4 bits (table-header part width) via gbs; hibits_zp is trashed but is
; re-seeded below before use.
        LDA #0
        LDY #4
        JSR gbs_lp
; Acc = 4 bits.
...
; C = 0, Y = 0
        TYA
        STA hibits_zp           ; hibits = 0 (stays 0 through the length read:
                                ; length part widths are <= 8, so only 0-bits
                                ; ever shift out of A into hibits)
...
; Y = 0, hibits_zp = 0 (still, from dc_skp0)
        TYA
        LDY bits_offs,X
        JSR gbs_entry
...
; (Y counts len-1..1 with a separate Y=0 tail: len can be 256, so a
;  DEY/BPL descending loop would exit early for len > 128.)
copy:
        LDY len_zp
        BEQ dc_skp4
dc_lp4:
