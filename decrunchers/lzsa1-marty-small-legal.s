; ===========================================================================
; Legal-only variant of lzsa1-marty-small.s (no undocumented opcodes): the one
; `ALR #$70` illegal op is expanded to the legal pair `AND #$70` / `LSR`
; (identical A, Z and C). Use this when the decruncher must run on a CPU or
; emulator without NMOS illegal opcodes. Decodes the same stream
; (lzan::lzsa1::compress_lzsa1).
; Upstream: decompress_small_v1.asm (c) 2019 Emmanuel Marty, zlib.
;
; The match-copy block sits above the token loop so its exit falls through
; into DECODE_TOKEN; the token's long-offset flag is ASLed into carry so one
; JSR GETSRC serves both offset paths. Entry keeps an out-of-loop `LDY #$00` /
; always-taken `BEQ DECODE_TOKEN` hop so every branch stays in short reach.
;
...
;@variant: legal
;@entry: full_decomp
;@vfy-key: lzsa1-marty-small-legal
...
; --- Entry: Y := 0 once, then hop over the match-copy block into the token
; loop. Y is 0 again on every loop re-entry, so the stub never re-runs. ------
full_decomp:
        LDY #$00
        BEQ DECODE_TOKEN                ; always taken (Z=1 from LDY #0)
...
DECODE_TOKEN:
        ...
        LSR                             ; (token&$70)>>1; Z set iff LLL==0
