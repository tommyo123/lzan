; ===========================================================================
; upkr 6502 decruncher, BACKWARD / in-place variant, in asm6502 syntax.
; Upstream: unupkr.asx (c) 2024 Piotr Fusik, zlib.
;
; Mirrors the forward decoder upkr-pfusik.s with only the direction aspects
; flipped:
;
...
;
; The rANS state machine, probability model, bit reader polarity and length
; decoding are byte-for-byte identical to the forward decoder: the encoder
; emits reverse(forward stream of reversed input)
; (lzan::upkr::compress_upkr_6502_backward), so the descending byte reader
; reproduces the forward byte sequence exactly.
;
...
; structurally from its `ADC u_dest+1` (a negative byte plus dest_hi >= $02
; always carries); the backward offset math adds small positive values, so the
; carry must be set explicitly.
;
