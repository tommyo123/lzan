; ===========================================================================
; Legal-only variant of lzsa2-marty-small-backward.s (no undocumented opcodes),
; mirroring its structure (assembly-time SMC seeding, shared count-prepare
; tail, EOD bail-out before the offset subtraction, DEX for LDX #$FF). Each
; illegal opcode is replaced by its shortest legal equivalent:
;   * ALR #$18 -> AND #$18 / LSR (identical A, Z and C);
;   * the DCP-based pointer decrements in PUTDST / GETSRC / the match-copy
;     step -> the classic LDA lo / BNE / DEC hi / DEC lo shape.
; Decodes the same stream (lzan::lzsa2::compress_lzsa2_anchor_backward).
; Use when illegal ops are unwanted.
...
;@variant: legal
;@entry: full_decomp
;@vfy-key: lzsa2-marty-legal-backward
...
GETSRC:                                 ; pre-decrementing read; preserves the
        LDA GETSRC_LDA+1                ;   carry every SBC caller relies on
        BNE GETSRC_DEC                  ;   (LDA/DEC/BNE leave C untouched)
        DEC GETSRC_LDA+2
GETSRC_DEC:
        DEC GETSRC_LDA+1
GETSRC_LDA:
