; ===========================================================================
; TSCrunch 6502 decruncher, BACKWARD / IN-PLACE variant (SAVON canonical).
; Upstream: TSCrunch decrunch.asm (c) 2022 Antonio Savona, Apache-2.0.
; Uses illegal ops (LAX/ALR), a shared LZ copy engine, and tail-merged exits.
;
...
; "backward" format is decoded FORWARD (ascending) - it is NOT a
; reverse-in/reverse-out descending stream. The token loop is IDENTICAL to the
; forward savon opt-size baseline; the only in-place work is (a) an in-place
; header preamble that ignores the dest address embedded in the stream and uses
...
;   * tsget is seeded directly to the first token (comp_data+6); the old
;     inc_get preamble hop is gone (lz2 no longer JMPs there either).
...
;     by the CPY-equal loop exit / ALR; Z set by DEY-to-zero) - no JMPs at all.
...
rle_tail:
        ...
        BCS updatezp_noclc       ; always taken (carry set from ALR / CPY exit)
