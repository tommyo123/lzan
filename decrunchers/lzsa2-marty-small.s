; ===========================================================================
; LZSA2 raw-block 6502 decruncher (size-optimized), in asm6502 syntax.
...
; (@smc: yes): source=comp_data, dest=out_addr and the rep-offset seed $FFFF
; are baked into the self-modifying operand bytes at assembly time, so there is
; no runtime init preamble. NIBCOUNT = $FC is a fixed ZP byte; NIBBLES patches
; the `LDA #$AA` immediate in GETNIBBLE.
;
