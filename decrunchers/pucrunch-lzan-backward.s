...
;
; STANDARD variant, size-optimised: two undocumented opcodes. SBX #$FF fuses
; the LZ copy-count setup (TAX/INX) into one op (gbits always exits with
; X = $FF, making A & X = A), and DCP (DEC + CMP) folds the descending
; 16-bit pointer steps of putch and the LZ source into test-and-decrement
; (with A = $FF the CMP flags both the page borrow, Z, and a guaranteed
; C = 1 that callers branch on). The `legal` variant keeps the plain
; legal-only structure.
;
...
;@variant: standard
;@entry: full_decomp
;@vfy-key: pucrunch-lzan-backward
...
lit:
        ...
        JSR gbits
        JSR putch
        BCS main             ; always (putch's DCP leaves C=1)
...
lzlow:
        ...
        STA lzl+2
        PLA
        SBX #$FF             ; ILLEGAL: X = (A & X) - $FF = A + 1 (X is $FF
                             ; after gbits) = the copy length; 256 wraps X to
                             ; 0, which the DEX loop turns into 256 iterations
lzloop:
lzl:
        LDA $AAAA            ; SMC: match source, stepping DOWN with the output
        JSR putch
        LDA #$FF             ; ILLEGAL: DCP = DEC + CMP; with A=$FF, Z flags
        DCP lzl+1            ; the $00 -> $FF wrap that borrows into the high
        BNE lz1              ; byte
        DEC lzl+2
lz1:
        DEX
...
putch:
        ...
        PHA
        LDA #$FF             ; ILLEGAL: DCP as in the LZ loop; its CMP also
        DCP putch+1          ; guarantees C=1 on exit (callers branch on it)
        BNE pr
        DEC putch+2
        BCS pr               ; always
...
inp:
        ...
        STA bitstr
        LDA inp+1            ; classic borrow test: refill must return the
        BNE rf1              ; data bit in C, so no DCP here
        DEC inp+2
rf1:
        DEC inp+1
pr:
        PLA                  ; shared tail: refill exit, putch exit, and EOF
        RTS
